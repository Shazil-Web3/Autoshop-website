...
export default function UsersClient({ initialRole }) {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      try {
        ...
        const filtered = initialRole ? (data || []).filter(u => u.role === initialRole) : (data || []);
        setUsers(filtered);
      } catch {
        setUsers([]);
      } finally {
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
          ...
          <p className="text-gray-600">Listing {initialRole ? `filtered by role: ${initialRole}` : 'all users'}</p>
        </div>
